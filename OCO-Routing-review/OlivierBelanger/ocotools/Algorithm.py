...
from .Problem import ConvNetFlow, ExpNetFlow
...
class IPM(Algorithm):
    ...
    def update(self, prob):
        ...
        if norm > 1 and self.damped:
            delta = delta/norm
        # print("norm = ", norm)
        norm2 = grad.T@np.linalg.inv(H)@grad
        print("norm = ", norm2)
        self.x -= delta[:self.n]
        self.lamda = delta[self.n:]
        # print("Time elapsed, IPM update: ", time.time()-start_time)
        return self.x
    ...
    def etaUpdateLimit(self, prob, beta=1.02, lim = 1e4):
        self.update(prob)
        self.eta = min(self.eta*beta, lim)
        return self.update(prob)
...
class IPMFeasible(IPM):
    ...
    def update2(self, prob):
        ...
        # rank_hth = np.linalg.matrix_rank(H.T@H)
        # print("rank_hth = ", rank_hth)
        # rank_htt = np.linalg.matrix_rank(H@H.T)
        # print("rank_htt = ", rank_htt)
        # print("H.shape[0] = ", H.shape[0])

        D = sp.block_array([[H, A.T],[A, None]]) 

        # rank_ddt = np.linalg.matrix_rank(D.todense())
        # print("rank_ddt = ", rank_ddt)
        # print("D.shape[0] = ", D.shape[0])
        # print("self.eta = ", self.eta)
        # print("prob.grad(self.x) = ", prob.grad(self.x))
        grad = grad + self.eta*prob.grad(self.x)
        viol = A.dot(self.x.T)-b.T
        viol.data = np.round(viol.data, 8)
        augGrad = sp.vstack((grad, viol))
