...
Interior-point methods do not attempt to solve \eqref{prob1} directly. Instead, a sequence of convex optimization problems related to the original problem are solved.
\WZZsuggestion{Although this is correct, I think it would be more precise to say:
"a sequence of equality-constrained quadratic minimization problems related to [...]"}
This is achieved in the following way. Let $\nn\in \mathbb{R}^+$ be a positive barrier parameter. This barrier parameter is useful in defining the following equality-constrained problem:
...
\begin{enumerate}
    ...
    The complexity of standard barrier functionals can be reasonably bounded by the dimension of the decision space, i.e., $v_f=O(N)$~\cite{boyd}. For example, the barrier functionals used for the space of 
    positive semi-definite (PSD) matrices \WZZsuggestion{maybe can say cone of PSD matrices, for consistency with the first paragraph of "A. Definitions"?},
    second-order cones, quadratic functions, and linear functions 
    \WZZ{this is slightly imprecise. there is no such thing as a barrier for a function. maybe you can use the wording: barrier for quadratic (convex) and linear inequality constraints instead.} all satisfy 
    \eqref{def:sccomplexity}~\cite[Section 2.3]{renegar}. \textcolor{Blue}{Of note, this implies that on the set $\Df$, $\phi(\xx)$ is strongly convex~\cite{boyd, renegar}.}

    \item \textcolor{Blue}{For $\xx \in \Df$, we have: \[\norm{\begin{bmatrix} \nabla^2 \phi(\xx) & \AA^\top\\ \AA& 0\end{bmatrix}} \le \frac{1}{m},\]
    \WZZtypo{missing an inverse here to be the same as the references.}
    where $m>0$ \WZZsuggestion{Should we use $K$ or $M$ instead of $1/m$ now that there is no inverse? 
    This is what the references use}.
    This assumption is similar to~\cite[Chapter 10]{boyd} and~\cite{InteriorOnline}.}
...
\WZZsuggestion{I would suggest to use $L$ for the Lagrangian, as it is more common than $\Gamma$ in the literature.}
The Lagrangian function has the following interesting properties \WZZsuggestion{maybe we can just say they are the gradient and hessian of the lagrangian?}:
\begin{align*}
...
We remark that $\DD(\yy)$ is the gradient of $r_t$ \WZZsuggestion{we don't need to say this? only the remaining of the sentence is important}
and is independent of both time $t$ and barrier parameter $\nn$. %The Hessian norms in the sequel will be defined with respect to this Lagrangian function, i.e. $\norm{\yy_1}^2_{\yy_2} = \yy_1^\top\DD(\yy_2)\yy_1,\quad\forall \yy_1,\yy_2\in \Df.$
...
the third derivative of $d_\nn (\xx)$ is bounded on $\Df$ by Assumption 3, the third derivative of $\Gamma_\nn^t(\yy)$ is also bounded.
\WZZ{this sentence is correct, but leads to the incorrect conclusion in the following sentence.
self-concordance is not just about bounding the third derivative, but about bounding it in terms of the second derivative.
More precisely, the third derivative must be bounded by a multiple of the 3/2 power of the second derivative, which is not well-defined
for non-convex functions.}}
Therefore, we have by~\cite[Section 2.5]{renegar}, that $\Gamma_\nn^t(\yy)$ is self-concordant 

\WZZ{this conclusion is incorrect. The reference mentions that an assumption made is that $H(x)$ is positive definite,
where $H$ is the Hessian of the function under consideration.
This is not the case for $\Gamma_\nn^t(\yy)$, since $\DD(\yy)$ is not positive definite, hence $\Gamma_\nn^t(\yy)$ cannot be self-concordant.}

\WZZ{You have suggested to use $\DD^2(\yy)$ instead, which would indeed be positive definite, 
but that can only tell us something about the self-concordancy of $\Gamma_\nn^t(\yy)^2$.
In addition, the current analysis and the algorithms you present are done with a Newton step defined with $\DD(\yy)$,
which wouldn't make much sense with $\DD^2(\yy)$ instead.}

\WZZ{To add to this, I found a recent reference on non-convex self-concordant functions:
"Non-Convex Self-Concordant Functions: Practical Algorithms and Complexity Analysis" 
by Goldfarb, et al. 2025.

Link: https://arxiv.org/abs/2511.15019.

I include a relevant quote from their introduction:

"Despite its success in convex optimization, extending self-concordance to non-convex settings
remains nontrivial. Although its definition appears superficially independent of convexity, it involves
the square root of local curvature terms, which can become ill-defined when the Hessian has negative
eigenvalues. One might attempt to circumvent this by taking absolute values before square roots, but this
modification imposes restrictive curvature bounds that fail to capture simple non-convex structures; for
example, the sigmoid function violates this property."

I think that this suggests there are potentially no simple remedies to the current analysis.}

\WZZsuggestion{I have a radical proposition to avoid using any kind of self-concordance analysis at all.
My understanding is that it is used to prove an upper bound on the number of Newton steps required to reach a certain accuracy
when solving the barrier problem at each time step.
This implies that multiple Newton steps are being taken per subproblem (which is true in standard interior-point methods).
However, in an online setting, we can only afford to take one Newton step per time step, since we have to move on to the next time step afterwards.
Therefore, I think we need to focus on the convergence of a single Newton step
and the properties of the solution being maintained in the region of convergence.}

The infeasible Newton's step, $n_t(\yy_t,\nn)$, \WZZsuggestion{I would remove the math here. and maybe use "def" for the definition?}
-- so-called because it does not require a feasible initial point -- at round $t$ associated to a barrier parameter $\nn$ can now be defined as:
...
\begin{lemma}\label{invHess}
    Let $\Gamma_\nn^t(\yy) = d_\nn(\xx)+\vv^\top(\AA\xx -\bb_t)$ 
    \WZZsuggestion{we can remove this redundant definition. or also could have said
    "let the Lagrangian be self-concordant"}
    be a self-concordant function. The following holds:
    \[\norm{\DD(\yy_1)\DD^{-1}(\yy_2)}_{\DD(\yy_1)}\le \frac{1}{(1-\norm{\yy_1-\yy_2}_{\DD(\yy_1)})^2}.\]
\end{lemma}
\WZZtypo{Should be $\DD(\yy)^{-1}$ instead of $\DD^{-1}(\yy)$, for the proof below as well.}
...
\end{proof}
\WZZ{the first equality is false. you cannot invert the matrix inside the norm with equality.
I read \cite[\S 2.5 Theorem 2.5.1]{renegar} (which is the result I think you are trying to generalize)
but there they have the relationship:
$\|H_x(y)^{-1}\|_x = \|H_y(x)\|_y$
which is slightly different because the argument to the hessian change,
there is no multiplication of two hessians, and the argument to the norms also change.}

\WZZ{the second equality is also not clear. where did the norms square appear from?
If you were doing the operator norm expansion of a matrix then it should have a sup, not a min.
For matrices, it is defined as the square root of the largest singular value, scaled by the hessian matrix.}

\WZZ{the third line is also not clear to me. you replaced min. by sup. and swapped the order of the two terms in the fraction.
this relationship is not correct. I think in general we can only guarantee that 
$\min (a/b) = 1/\max (b/a)$, but not that $\min(a/b) \leq \max(b/a)$.
I read \cite[\S 2.2.3]{renegar} (which I assume is what you are referring to) 
but renegar uses a fixed vector v to get the inequality, and the min. is over
the indices of barrier functionals ($i.e. f_1, f_2$) in a summation, not over the direction v.}

\WZZsuggestion{I would suggest to say "where the last inequality follows from 
squaring both sides of \eqref{def:selfconcordance}" at the end of your proof
to more precisely describe what you are doing.}
%\begin{lemma}
...
The dual-feasible nature of every point on the central path provides an estimation of the error for each iteration 
\WZZ{I am not sure what you are referring to here. Lemma 2 doesn't use anything related to dual feasibility.
Also, dual feasibility provides a lower bound on the optimal objective value, 
I am not sure how it estimates the error of each iteration?}.
Defining the next iterate as $\yy^+ =\yy + n_t(\yy,\nn)$, Lemma~\ref{lem:nred} formalizes this concept. 
\begin{lemma}
\label{lem:nred}
    For any pair $\{\yy,\nn\}$ such that $\normy{n_t(\yy,\nn)}\le 1$, we have:
...
\WZZ{ditto. This is not the hessian norm but rather its square.}:
\begin{align*}
...
\end{align*}
\WZZtypo{missing a subscript t on the first line for $\Delta \mathbf{r}$.}
\WZZ{second line seems to be the matrix version of the inequality $(a+b)^2 \le 2a^2 + 2b^2$. However, it is only true if 
$\DD(\yy)^{-1}$ is positive definite, which is not the case. We need to be more careful here.}
\textcolor{Blue}{where we used Assumption 4 and the bound on $\norm{\Delta\bb}$ to obtain the final inequality}
\WZZ{fourth line should be an inequality, not an equality. also it is only true if we have $m > 1$.}
\end{proof}
%
...
From~\cite[Section 2.3.1]{renegar}, we have that the restriction of any barrier functional to a subspace or translation of a subspace results in a barrier functional whose complexity barrier is smaller than that of the original functional.
\WZZsuggestion{I could not find the reference in the section in question.
Are you talking about Theorem 2.3.2? If so, the context seems slightly different.}

\WZZ{Also, what we are doing below is not a restriction to a subspace, but rather zeroing out some gradient components
(those corresponding to the primal feasibility).
Are we implying that these points are feasible and satisfy $Ax=b_t$?
If anything, this relationship seems to operate in the superspace (of the primal-dual variables),
not a subspace (like in Renegar, which stays in the primal space restricted to ${x: Ax=b_t}$).}
...
\WZZ{Also the claimed bound doesn't hold for arbitrary dual variable $\vv$.
As $\|\vv\| \to \infty$, the quadratic form in the left hand side will
becomes unbounded due to cross-terms.
The complexity bound $v_f$ only applies to the barrier functional in the primal variables,
not to arbitrary dual variables.}

\WZZsuggestion{maybe this will be true for either dual feasible points, or for dual variables
that are optimal for the current primal variable $\xx$.
We should add a clarifying sentence before we mention "any pair".}
...
\WZZtypo{should it be $\DD(\yy)^{-1}$ instead of $\DD(\xx)^{-1}$, here for consistency with the definition
...
\begin{proof}
    ...
    \WZZ{again we need to be careful. This is not the hessian norm but rather its square.
    Also the inequality holds after taking the square root only if both sides are non-negative, which is not
    necessarily the case for the left-hand side since $\DD(\yy_t)$ is not positive definite.}
    
    \WZZ{I looked at this further and found another potential issue, even if $\DD(\yy_t)$ were positive definite.
    The inequality on the second line above can be simplified to the following (after expanding and simplifying):
    $2(\xx_t-\xx_t^\nn)^\top \AA^\top (\vv_t - \vv_t^\nn) \geq 0$.
    However, this inequality is not true for any primal-dual feasible point $\yy$.}
\end{proof}

\section{Algorithms} \label{algos}

Suppose we have an initial primal-dual point $\xx_0$, $\vv_0$ and an initial barrier parameter $\nn_0$ such that $\norm{n_0(\yy_0, \nn_0)}_{\DD(\yy_0)} \le \frac{1}{9}$ and $\AA\xx_0=\bb_0$. \textcolor{Blue}{In practice, this can be achieved using offline optimization before the start of the online procedure.}
\WZZsuggestion{This is basically a requirement of feasibility and proximity to the central path (hence the small newton step).
Discussions with Antoine said that an offline problem is solved to optimality to find such a point.
Is there a simpler approach to find such a point, rather than solving an entire optimization problem?
Maybe we can consider Phase 1 feasibility methods?}.
We propose the online interior-point method for time-varying equality constraints (\texttt{OIPM-TEC}) presented in Algorithm~\ref{alg:TIP2}.
...
\begin{figure}
    \centering
        \includegraphics[width=0.45\textwidth]{images/regretFig.pdf}
        \vspace{-0.5cm}
        \caption{Regret comparison of \texttt{OIPM-TEC}, \texttt{$\epsilon$OIPM-TEC}, and \texttt{MOSP}}
        \label{fig:regret2}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{images/const3}
    \vspace{-0.5cm}
    \caption{Constraint violation comparison of \texttt{OIPM-TEC}, \texttt{$\epsilon$OIPM-TEC}, and \texttt{MOSP}}
    \label{fig:const2}
    \vspace{-0.5cm}
\end{figure}
...
\[\norm{n_t(\yy, \nn)}_{\DD(\yy)}^2 = r_t(\yy,\nn)^\top\DD(\yy)^{-1}r_t(\yy,\nn).\]
We can express $\norm{n_t(\yy^+,\nn)}_{\DD(\yy^+)}$ \WZZtypo{I think you are missing a square here} as:

\WZZsuggestion{maybe we can directly go to the derivations and avoid the sentence above?
We would need to change the equations to be directly in terms of $y^+$ though.}
\begin{align*}
    &\norm{n_t(\yy^+,\nn)}^2_{\DD(\yy^+)} = r_t(\yy^+,\nn)^\top\DD(\yy^+)^{-1}r_t(\yy^+,\nn)\\
...
Using Assumption 4 \WZZtypo{is this a typo? did you mean Lemma 1 instead?},
...
\WZZtypo{should the argument to the hessian be $\yy$ instead of $\xx$, for consistency?}
This allows us to express $n_t(\yy, \nn^+)$ as a function of $n_t(\yy, \nn)$ 
\WZZsuggestion{If its possible, it would nice to describe this relationship using english.
Example: "This allows us to express the newton's step taken at $\eta^+$ as a function of the previous newton's step."}:
\[n(\yy,\nn^+) = \frac{\nn^+}{\nn}n_t(\yy,\nn) + \left(\frac{\nn^+}{\nn}-1\right)h(\yy).\]
\WZZtypo{missing a subscript $t$ in the first $n$ above.}
